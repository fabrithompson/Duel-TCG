...
import { createUserWithEmailAndPassword, deleteUser, signOut, updateProfile, UserCredential } from 'firebase/auth';
...
function validar(nombre: string, email: string, password: string, codigo: string, necesitaCodigo: boolean): Errores {
...
export default function RegisterScreen() {
  ...
  const crear = async () => {
    ...
    const nuevos = validar(nombreLimpio, mail, password, codigoLimpio, necesitaCodigo);
    setErrores(nuevos);
    setErrorGeneral(null);
    ...
    try {
      ...
      try {
        ...
        if (c === 'auth/email-already-in-use' || c === 'auth/invalid-email') fallar({ email: mensajeError(e) }, null);
      ...
      await updateProfile(cred.user, { displayName: nombreLimpio }).catch(() => undefined);

      try {
        await setDoc(doc(db, 'users', cred.user.uid), {
          uid: cred.user.uid,
          nombre: nombreLimpio,
          email: cred.user.email ?? mail.toLowerCase(),
          role,
        ...
        // Sin perfil la cuenta de Auth queda huérfana y bloquea el email: se borra para poder reintentar.
        await deleteUser(cred.user).catch(() => signOut(auth).catch(() => undefined));
        const denegado = codigoError(e)?.replace(/^firestore\//, '') === 'permission-denied';
        if (denegado && necesitaCodigo) fallar({ codigo: CODIGO_INVALIDO }, null);
        else fallar({}, mensajeError(e, 'No pudimos guardar tu perfil. Probá de nuevo.'));
        return;
      }
    ...
    <SafeAreaView style={[styles.flex, { backgroundColor: colors.bg }]} edges={['top', 'bottom']}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <Button
            label={`Crear cuenta de ${ROLE_LABEL[role]}`}
            onPress={() => {
              void crear();
            }}
            loading={enviando}
